...
- Phase 5 prepares launch operations with secure inquiry and booking-request flows, route metadata, structured data, analytics hooks, legal pages, sitemap/robots files, and a branded 404. Outbound email remains gated by Resend environment configuration; final-domain activation is still deferred.
- The verified production deployment is available at `https://symbolic-development-design-system.vercel.app`; this Vercel address remains the canonical staging/launch URL until the final domain is provided.
...
- Production responses ship security headers from `vercel.json` (CSP, `X-Frame-Options`, `nosniff`, referrer/permissions policies, COOP/CORP, HSTS). Keep the CSP aligned with self-hosted assets plus Vercel Analytics script/connect hosts.
- Contact API rate limiting uses a shared store abstraction: Upstash Redis when `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` are set (preferred on Vercel), otherwise in-memory fallback. Limits are 5 requests / 15 minutes per IP and 3 requests / hour per email, with `Retry-After` on 429 responses.
