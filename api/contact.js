...
export default async function handler(request,response) {
  ...
  const attribution={source:clean(body.utm_source),medium:clean(body.utm_medium),campaign:clean(body.utm_campaign),content:clean(body.utm_content),term:clean(body.utm_term),landingPath:clean(body.landingPath),referrer:clean(body.referrer)};
  if([preferredWindow,stage,timeline,engagement,budget].some(value=>value.length>120) || referral.length>80 || Object.values(attribution).some(value=>value.length>240)) return response.status(400).json({message:"Please check the highlighted information and try again."});
  ...
  const attributionDetails=Object.values(attribution).some(Boolean)?`<p><strong>Acquisition context</strong><br><strong>Source:</strong> ${escapeHtml(attribution.source)||"Direct"}<br><strong>Medium:</strong> ${escapeHtml(attribution.medium)||"Not provided"}<br><strong>Campaign:</strong> ${escapeHtml(attribution.campaign)||"Not provided"}<br><strong>Content:</strong> ${escapeHtml(attribution.content)||"Not provided"}<br><strong>Term:</strong> ${escapeHtml(attribution.term)||"Not provided"}<br><strong>Landing page:</strong> ${escapeHtml(attribution.landingPath)||"Not provided"}<br><strong>Referrer:</strong> ${escapeHtml(attribution.referrer)||"Not provided"}</p>`:"";
  const result=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({from,to:[to],reply_to:email,subject:kind==="booking"?`Working session request — ${name}`:`New project inquiry — ${name}`,html:`<div style="font-family:Arial,sans-serif;max-width:640px"><h1>${kind==="booking"?"Working session request":"Project inquiry"}</h1><p><strong>Name:</strong> ${escapeHtml(name)}<br><strong>Email:</strong> ${escapeHtml(email)}<br><strong>Company:</strong> ${escapeHtml(company)||"Not provided"}</p>${details}<p><strong>Context</strong></p><p style="white-space:pre-wrap">${escapeHtml(message)}</p>${attributionDetails}</div>`})});
