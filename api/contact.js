...
export default async function handler(request,response) {
  ...
  if(recent.length>=5) return response.status(429).json({message:"Too many requests. Please wait a few minutes and try again."});
  recent.push(now); limits.set(ip,recent);
  let body; try{body=typeof request.body==="string"?JSON.parse(request.body):request.body||{};}catch{return response.status(400).json({message:"Invalid request."});}
  ...
  const preferredDate=clean(body.preferredDate),preferredWindow=clean(body.preferredWindow);
  if(kind==="booking" && (!/^\d{4}-\d{2}-\d{2}$/.test(preferredDate) || !preferredWindow)) return response.status(400).json({message:"Choose a preferred date and time window."});
  const apiKey=process.env.RESEND_API_KEY,to=process.env.CONTACT_TO_EMAIL,from=process.env.CONTACT_FROM_EMAIL;
  if(!apiKey || !to || !from) return response.status(503).json({message:"Secure email delivery is being connected. Please email us directly for now."});
  const details=kind==="booking"?`<p><strong>Preferred date:</strong> ${escapeHtml(preferredDate)}<br><strong>Preferred window:</strong> ${escapeHtml(preferredWindow)}</p>`:"";
  ...
  const data=await result.json().catch(()=>({})); if(!result.ok) return response.status(502).json({message:"Email delivery failed. Please email us directly.",detail:data.message});
