...
const schedulingUrl=(()=>{try{const url=new URL(import.meta.env.VITE_SCHEDULING_URL||"");return url.protocol==="https:"?url.href:"";}catch{return "";}})();

function readAcquisitionContext() {
  try {
    const stored=JSON.parse(window.sessionStorage.getItem("sdds:acquisition")||"{}");
    const query=new URLSearchParams(window.location.search);
    const context={...stored};
    for(const key of attributionKeys) if(query.get(key)) context[key]=query.get(key).slice(0,160);
    if(!context.landing_path) context.landing_path=`${window.location.pathname}${window.location.search}`.slice(0,240);
    if(!context.referrer&&document.referrer){const referrer=new URL(document.referrer);if(referrer.origin!==window.location.origin)context.referrer=referrer.href.slice(0,240);}
    window.sessionStorage.setItem("sdds:acquisition",JSON.stringify(context));
    return context;
  } catch { return {}; }
}

function AcquisitionFields({ referral="" }) {
  const context=readAcquisitionContext();
  return <>{referral&&<input type="hidden" name="referral" value={referral}/>} {attributionKeys.map(key=><input key={key} type="hidden" name={key} value={context[key]||""}/>)}<input type="hidden" name="landingPath" value={context.landing_path||""}/><input type="hidden" name="referrer" value={context.referrer||""}/></>;
}
...
function Contact() {
  ...
  const send=async event=>{event.preventDefault();const form=event.currentTarget;const attribution=readAcquisitionContext();setStatus("sending");setError("");try{await submitLead(form,"contact");trackEvent("contact_submitted",{source:attribution.utm_source||"direct",campaign:attribution.utm_campaign||"none"});setStatus("sent");}catch(reason){trackEvent("contact_submit_failed",{source:attribution.utm_source||"direct"});setError(reason.message);setStatus("error");}};
...
const policyContent={
  privacy:{eyebrow:"PRIVACY",title:"Clear handling of the information you share.",updated:"AUGUST 12, 2026",sections:[["Information we collect","We collect the information you choose to provide through an inquiry or booking request, including your name, email address, company, project context, and preferred meeting window. We may also receive limited technical data and campaign attribution parameters needed for security and aggregate site measurement."],["How we use it","We use inquiry information to respond, prepare for conversations, understand which outreach led to a request, protect the site from abuse, and improve how the website supports prospective clients. We do not sell personal information."],["Service providers","Hosting, email delivery, scheduling, and privacy-respecting analytics providers may process limited information on our behalf under their own security and privacy commitments."],["Retention and choices","We retain inquiry information only as long as reasonably necessary for business communication, legal obligations, and security. You may request access, correction, or deletion by emailing hello@symbolicdev.com."]]},
...
function BookingPage() {
  ...
  const send=async event=>{event.preventDefault();const form=event.currentTarget;const attribution=readAcquisitionContext();setStatus("sending");setError("");try{await submitLead(form,"booking");trackEvent("booking_requested",{source:attribution.utm_source||"direct",campaign:attribution.utm_campaign||"none",referral:referral||"none"});setStatus("sent");}catch(reason){trackEvent("booking_request_failed",{source:attribution.utm_source||"direct",referral:referral||"none"});setError(reason.message);setStatus("error");}};
