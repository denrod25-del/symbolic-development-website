...
function ButtonLink({ href,children,primary=false }) { return <SiteLink className={`button ${primary ? "button-primary" : "button-secondary"}`} href={href}><span>{children}</span><ArrowUpRight size={16} /></SiteLink>; }
...
const policyContent={
  privacy:{eyebrow:"PRIVACY",title:"Clear handling of the information you share.",updated:"AUGUST 07, 2026",sections:[["Information we collect","We collect the information you choose to provide through an inquiry or booking request, including your name, email address, company, project context, and preferred meeting window. We may also receive limited technical data needed for security and aggregate site measurement."],["How we use it","We use inquiry information to respond, prepare for conversations, protect the site from abuse, and improve how the website supports prospective clients. We do not sell personal information."],["Service providers","Hosting, email delivery, and privacy-respecting analytics providers may process limited information on our behalf under their own security and privacy commitments."],["Retention and choices","We retain inquiry information only as long as reasonably necessary for business communication, legal obligations, and security. You may request access, correction, or deletion by emailing hello@symbolicdevelopment.com."]]},
...
function BookingPage() {
  ...
  const send=async event=>{event.preventDefault();const form=event.currentTarget;setStatus("sending");setError("");try{await submitLead(form,"booking");trackEvent("booking_requested");setStatus("sent");}catch(reason){setError(reason.message);setStatus("error");}};
